(function ($) {
    ...
    function generarCodigo() {
      console.log("Generar Codigo Informe intermedio");
  
      $(".field-propuesta_vinculada select").change(function () {
        let count = 0
        let url = "/countinformeintermedio/";
        let paiscode = "";
        let duracionProp = ""
        paiscode = $(this).val();
        url += paiscode;
        ...
        $.ajax({
          ...
          success: function (data) {
            ...
            let codigo_org = $(".field-propuesta_vinculada select option:selected").text();
            suffix  = ""
            if (duracion=='None'){
              suffix = ''
              codigo_org = ''
              alert('La propuesta seleccionada no tiene definida su duración. No se puede crear un infrome hasta que la propuesta tenga su duración definida.')
            } else if (count==0 && duracion=='12M') {
              if(count==0){
                suffix = '-I.Intermedio(6M)'
                codigo_org = codigo_org + suffix;
              } else if (count>=1){
                suffix = ''
                codigo_org = ''
                alert('La propuesta de 12 meses ya tiene un informe intermedio existente. No puede tener más. Agregue un informe final o cambie la duración de la propuesta.')
              }
              ...
              if(count==0){
                ...
                suffix = '-I.Intermedio(12M)'
                codigo_org = codigo_org + suffix;
              } else if (count>=2){
                suffix = ''
                codigo_org = ''
                alert('La propuesta de 18 meses ya tiene dos informe intermedio existente. No puede tener más. Agregue un informe final o cambie la duración de la propuesta.')
              }
              ...
              if(count==0){
                ...
                suffix = '-I.Intermedio(18M)'
                codigo_org = codigo_org + suffix;
              } else if (count>=2){
                suffix = ''
                codigo_org = ''
                alert('La propuesta de 24 meses ya tiene dos informe intermedio existente. No puede tener más. Agregue un informe final o cambie la duración de la propuesta.')
              }
    ...
    $(document).ready(function () {
      console.log("Informeintermedio create!");
      generarCodigo();
